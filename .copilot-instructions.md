...
- **QNNN**: Quantum Neural Network Notation (passenger capacity in this context)
...
### Optimization System
- `scripts/qaoa_over_F.py` - QAOA optimization implementation over feasible set
- `constraints/hard_constraints.yaml` - TRL gates, compatibility matrices, physics bounds
- `data/candidates.yaml` - AMPEL donor aircraft subsystem database
...
```python
# Uses MILP/CP-SAT to enforce hard constraints
...
```bash
python3 scripts/qaoa_over_F.py --optimize  # Run QAOA optimization
