...
/**
 * AMPEL360 H₂-BWB-Q Framework Generator
 * Main executable script
 */

import { generateFrameworkStructure, frameworkToJSON, countNodes } from './framework-generator';
import { FrameworkGeneratorOptions } from './types';
 ...
 */
function main() {
  console.log('🚀 AMPEL360 H₂-BWB-Q Framework Generator');
  console.log('==========================================');
  
  const options: FrameworkGeneratorOptions = {
    includeFolders: true,
  ...
  try {
    ...
    const nodeCount = countNodes(framework);
    console.log(`✅ Generated framework with ${nodeCount} nodes`);
    
    // Output to JSON file
    const jsonOutput = frameworkToJSON(framework);
    console.log('\n📄 Framework structure (JSON):');
    console.log(jsonOutput);
    
    // Summary statistics
    console.log('\n📈 Framework Statistics:');
    console.log(`- Total nodes: ${nodeCount}`);
    console.log(`- Root node: ${framework.name}`);
    console.log(`- Top-level domains: ${framework.children?.length || 0}`);
    
    if (framework.children) {
      console.log('\n🏗️ Domain breakdown:');
      for (const domain of framework.children) {
        const domainNodeCount = countNodes(domain);
        console.log(`  - ${domain.name}: ${domainNodeCount} nodes`);
      }
    }
